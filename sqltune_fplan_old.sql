...
-- Input the sql_id and the requested plan. 
--
-- DESCRIPTION
...
EXEC 	DBMS_OUTPUT.PUT_LINE(chr(10)||'***INFO***:	The requested plan must be searched in AWR (DBA_HIST_SQLSTAT)')
--ACCEPT	ref_date DATE FORMAT 'dd/mm/yyyy' PROMPT 'Specify the Reference date from which backwards you want to search the AWR (enter for sysdate):' 
PROMPT
ACCEPT	days_back NUMBER PROMPT 'Specify the number of days back from SYSDATE for which you want to search the AWR:'
...
exec DBMS_OUTPUT.PUT_LINE(chr(10)||'***INFO***:	The requested plan has been loaded into the SQL Plan Baseline. Lets check DBA_SQL_PLAN_BASELINES ...')
...
declare
	...
	begin
		...
		select DBMS_SQLTUNE.SQLTEXT_TO_SIGNATURE (sql_text  =>l_sql_text, force_match => 0) into l_sign from dual;				
...
PROMPT
PROMPT And the plan is the following (note that the plan hash value might be different):
PROMPT
