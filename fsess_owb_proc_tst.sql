...
--
------------------------------------------------------
create or replace procedure monitor_dw.fsess_owb_proc_tst
IS
...
insert into monitor_dw.fsess_owb (
           SNAPSHOT_DT, "Problem / Action", OWB_FLOW, OWB_PARENT_FLOW,
           USERNAME, OWB_NAME, OWB_TYPE, 
           OWB_AUDIT_ID, OWB_CREATED_ON, OWB_UPDATED_ON, 
           OWB_STATUS, OWB_RESULT, DURATION_MINS, 
           OWB_DURATION_MINS_P80, OWB_TIMES_EXCEEDING_P80, INST_ID, 
           SID, SERIAL#, LOGON_TIME, 
           STATUS, PROG, SQL_ID, 
           SQL_CHILD_NUMBER, SQL_EXEC_START, PLAN_HASH_VALUE, 
           SQL_TEXT, ENTRY_PLSQL_PROC, WAIT_STATE, 
           WAIT_CLASS, EVENT, SECS_IN_WAIT, 
           OBJ_OWNER, OBJ_NAME, OBJ_TYPE, 
           BLOCKING_INSTANCE, BLOCKING_SESSION, BLOCKER, 
           BLOCKER_OWB_NODE, BLOCKER_MAIN_FLOW, KILL_BLOCKER_STMNT, 
           OSUSER, OSPROCESS, MACHINE, 
           PORT, TERMINAL, PREV_SQL_ID, 
           PREV_CHILD_NUMBER, PREV_EXEC_START, PREV_PLAN_HASH_VALUE, 
           PREV_SQL_TEXT
    )    
    with owb_execs
...
select dense_rank() over(order by username, owb_flow, owb_name ) task_no, t.* 
from monitor_dw.fsess_owb t
where 1=1
    --and owb_name = 'SOC_ORDER_LINE_ATTR_VER_LD_FCT'
    and snapshot_dt = to_date('20-08-2015 09:37:42', 'dd-mm-yyyy hh24:mi:ss')
order by username, owb_flow, owb_name

select dense_rank() over(order by username, owb_flow, owb_name ) task_no, t.* 
from monitor_dw.fsess_owb t
where 1=1    
    and snapshot_dt = to_date('20-08-2015 07:07:42', 'dd-mm-yyyy hh24:mi:ss')
order by username, owb_flow, owb_name  
******************/    
    ...
    for r in (
        select  snapshot_dt,
                "Problem / Action",
                case when "Problem / Action" in ('"LOW PERFORMANCE"','"OWB task in "BUSY" state, with no corresponding DB session (possible corruption) / Retry node from OWF Monitor"' ) then 'DEV_STANDBY_BI' ELSE 'AS_STANDBY_BI' end WHO2CALL,                
                owb_flow,
                owb_parent_flow,
                username,
                owb_name,
          ...
          from (
                    ...
                    from monitor_dw.fsess_owb t
                    where 1=1    
                        and snapshot_dt = to_date('04-02-2016 17:18:30', 'dd-mm-yyyy hh24:mi:ss')
                    order by username, nvl(owb_flow, sql_id), owb_name                 
                /*
                select dense_rank()over(order by username, owb_flow, owb_name ) task_no, t.* 
                from monitor_dw.fsess_owb t
                where 1=1
                    ...
                    where   snapshot_dt = l_target_dt -- snapshot_dt > sysdate -  2/60/24 --trunc(sysdate, 'MI') = trunc(snapshot_dt, 'MI'
                            ...
                            and ("Problem / Action", nvl(to_char(OWB_AUDIT_ID), SQL_ID)) not in ( --and ("Problem / Action", nvl(OWB_FLOW, username), nvl(OWB_NAME, sql_id)) not in ( -- dont send the same mail again, if it has been sent in the last 3 hours!
                                ...
                                where 
                                    ...
                                    AND trunc(snapshot_dt) = trunc(t.snapshot_dt) --snapshot_dt > sysdate - 180/60/24
                            )
    ...
    loop
        ...
        l_msg_tmp := 
                    ...
                    '|   Parent Flow:            '||r.owb_parent_flow||chr(10)||                    
    ...
    */          
    ...
    if (l_rows_returned = 1) then
        -- send mail
        monitor_dw.fsess_owb_send_mail_tst (l_message);                         
    end if;
    
END fsess_owb_proc_tst;
/

CREATE OR REPLACE procedure monitor_dw.fsess_owb_send_mail_tst (
...
--  l_mesg    CLOB; -- VARCHAR2(4000);
  l_subject     varchar2(100)   :=  'Test Ignore - *** Problem in DW Running Task  ***';
