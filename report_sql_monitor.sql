...
--
--              Note 3: This script is not very robust, so you may be better off taking
--                      the syntax and making your own version. ;) I often use a version
--                      that prompts for SID, SERIAL, SQL_ID and SQL_EXEC_ID (which can
--                      be found in v$sql_monitor.
---------------------------------------------------------------------------------------
set long 999999999
set lines 400
col report for a379
...
DBMS_SQLTUNE.REPORT_SQL_MONITOR(
   ...
   session_serial=>decode('&&sid',null,null,
       sys_context('userenv','sid'),(select serial# from v$session where audsid = sys_context('userenv','sessionid') and sid = '&&sid'),null),
   sql_id=>'&&sql_id',
