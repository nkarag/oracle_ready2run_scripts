...
-- in the history of interest (&days_back from sysdate), based on ***average DB time per execution*** as this is recorded in DBA_HIST_ACTIVE_SESS_HISTORY.
...
-- This way we can more easily identify "heavy" sql_ids and get the (average) DB time ***per execution***. Also we can measure
...
col p80_dbtimepx format 999G999G999G999
...
select
		row_number() over(order by p80_dbtimepx desc) r, --avg_dbtime_per_exec_sec desc ) r,
		dbt.p80_dbtimepx, dbt.avg_dbtime_per_exec_sec, dbt.dbtime_std_dev, dbt.min_dbtime_per_exec_sec, dbt.max_dbtime_per_exec_sec, dbt.numof_executions,
		avg_duration_mins,
		std_dev_duration,
		min_duration_mins,
		max_duration_mins,
...
from (  
	select /*+ parallel(16) */  
			fin.p80_dbtimepx, fin.avg_dbtime_per_exec_sec, fin.dbtime_std_dev, fin.min_dbtime_per_exec_sec, fin.max_dbtime_per_exec_sec, fin.numof_executions,
			fin.sql_id,
			fin.SQL_OPNAME, 
			fin.username,
	...
	from (
		select 
			topn.username, -- decode(topn.USER_ID, 3839, 'ETL_DW', 3980, 'PERIF', (select username from dba_users where user_id = topn.user_id)) username,
		...
		(
			select *
			from (
					select decode(t.USER_ID, 3839, 'ETL_DW', 3980, 'PERIF') username, --t.user_id, 
							t.PLSQL_ENTRY_OBJECT_ID, t.PLSQL_ENTRY_SUBPROGRAM_ID, t.sql_id, t.SQL_OPNAME, 
							PERCENTILE_CONT(0.8) WITHIN GROUP (ORDER BY DBtime_secs ASC) p80_dbtimepx,
					...
					group by decode(t.USER_ID, 3839, 'ETL_DW', 3980, 'PERIF'), --t.user_id, 
						t.PLSQL_ENTRY_OBJECT_ID, t.PLSQL_ENTRY_SUBPROGRAM_ID, t.sql_id, t.SQL_OPNAME
					order by p80_dbtimepx desc --avg_dbtime_per_exec_sec desc
			) where rownum <= '&n' 
		) topn
			left outer join dba_procedures p on (topn.PLSQL_ENTRY_OBJECT_ID = p.OBJECT_ID AND topn.PLSQL_ENTRY_SUBPROGRAM_ID = p.SUBPROGRAM_ID) -- AND topn.username = p.owner)
	...
	left outer join (
						...
						GROUP BY REGEXP_REPLACE(REGEXP_REPLACE(execution_name, '_\d\d$', ''),'\w+:' , '' ), DECODE (task_type, 'PLSQL', 'Mapping',  'PLSQLProcedure', 'Procedure', 'PLSQLFunction', 'Function')
	) owb 
	on ( dbt.owb_node_name = owb.exec_name_short  AND dbt.owb_object_type = owb.type)
order by p80_dbtimepx desc --avg_dbtime_per_exec_sec desc 
/		
