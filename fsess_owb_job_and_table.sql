...
job_action => 'begin
monitor_dw.fsess_owb_proc;
end;',
repeat_interval => 'FREQ=MINUTELY;INTERVAL=15',
start_date => systimestamp at time zone 'Europe/Athens',
job_class => '"DEFAULT_JOB_CLASS"',
comments => 'run fsess_owb every 15 minutes',
auto_drop => FALSE,
enabled => TRUE);
END;
...
exec dbms_scheduler.stop_job(job_name => 'MONITOR_DW.FSESS_OWB_JOB', force => TRUE)

-- disable the job
exec DBMS_SCHEDULER.DISABLE (name => 'MONITOR_DW.FSESS_OWB_JOB', force => TRUE)

-- enable the job
exec DBMS_SCHEDULER.ENABLE (name => 'MONITOR_DW.FSESS_OWB_JOB')
...
alter table monitor_dw.fsess_owb add owb_parent_flow varchar2(300);
