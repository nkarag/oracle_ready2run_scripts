...
and ash.SAMPLE_TIME > sysdate - (&minutes_from_now/(24*60))
and ash.sql_id = nvl('&&sql_id',ash.sql_id) and ash.SQL_CHILD_NUMBER = nvl('&&SQL_CHILD_NUMBER',0)
...
AND plan.sql_id LIKE nvl('&&sql_id',plan.sql_id) --'&1'
AND plan.child_number LIKE '%'||nvl('&&SQL_CHILD_NUMBER',0) --'%&2%'
ORDER BY
    plan.child_number
