----------------------------------------------------------------------------------------
--
-- File name:   spool2csv_hash.sql
--
-- Purpose:     Generates an sqlplus script and then runs it that spools a specific table to a CSV file. It can hash specific columns in order to encrypt them.
--				Note that this is a one way hashing with the Secure Hash Algorithm (SHA). Produces a 160-bit hash.
--
--
-- Version:     2016/11/19
--
-- Usage:       This script inputs 5 parameters. 
...
--				Parameter 5	is a list of columns that we need to hash
--
-- Example:     @spool2csv_hash.sql	
--
-- Notes: 	    
--				*Developed and tested on 11.2.0.1
--
--				*The generated script is called "do_spoolcsv_hash.sql" and it can be run seperately (you can 
--				  comment out the line where this script is called). However, if you want to run the generated do_spoolcsv_hash.sql as a 
...
-- get list of columns to be hashed
prompt 'enter list of column names to be hashed (separated by ","):'
def hash_col_list = '&5'
...
-- spool on
spool do_spoolcsv_hash.sql replace
