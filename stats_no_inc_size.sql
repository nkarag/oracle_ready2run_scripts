...
prompt find tables  where incremental statistics is not used  join with segment size ...
...
select noinc.owner, noinc.table_name, round(sum(bytes)/1024/1024/1024,2) GBs
from noinc join dba_segments s on (noinc.owner = s.owner AND noinc.TABLE_NAME = s.SEGMENT_NAME)
group by noinc.owner, noinc.table_name
order by GBs desc
/
