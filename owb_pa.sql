...
--
--		v2:	
--			1.	Step 2 has been broken into several parts (tmp tables for performance reasons)
--			2.	Added an "inline" hint at the critical path hierarchical query (create table monitor_dw.owb_etlpa2_tmp2b)
--			3.  Store the results of the high-level flow deviations to a table in MONITOR_DW for better historical performance monitoring
--
...
insert into monitor_dw.DWP_ETL_FLOWS_DEVIATIONS_HIST
    select sysdate, t.* from monitor_dw.owb_etlpa2_tmp1 t;
...
--
--		Note: Due to performance reasons the following query has been broken down to many tmp tables.
-----------------------------------------------------------
...
begin
    ...
    from dba_tables 
    where owner = 'MONITOR_DW'
        and table_name = upper('owb_etlpa2_tmp2a');

    if(l_table_exists > 0) then
        execute immediate 'drop table MONITOR_DW.owb_etlpa2_tmp2a';
    end if;
end;
/

-- drop table monitor_dw.owb_etlpa2_tmp2a;

create table monitor_dw.owb_etlpa2_tmp2a
as
with q1 
as ( 
...
--select count(*) from monitor_dw.owb_etlpa2_tmp2a;

exec dbms_stats.gather_table_stats('MONITOR_DW', upper('owb_etlpa2_tmp2a'))
...
begin
    ...
    from dba_tables 
    where owner = 'MONITOR_DW'
        and table_name = upper('owb_etlpa2_tmp2b');

    if(l_table_exists > 0) then
        execute immediate 'drop table MONITOR_DW.owb_etlpa2_tmp2b';
    end if;
end;
/

-- drop table monitor_dw.owb_etlpa2_tmp2b;

create table monitor_dw.owb_etlpa2_tmp2b
as
with t3
as (
    select /*+ qb_name(seq_path_qry) no_merge inline */ CONNECT_BY_ISLEAF "IsLeaf2",
         ...
         t2.duration_mins + prior t2.duration_mins dur_mins,
         T2.*
...
--select count(*) from monitor_dw.owb_etlpa2_tmp2b;

exec dbms_stats.gather_table_stats('MONITOR_DW', upper('owb_etlpa2_tmp2b'))
...
as (
    ...
    from monitor_dw.owb_etlpa2_tmp2b t3 --t3
    order by root_created_on desc, seq_length desc               
),
t5
as (
    select  /*+ qb_name(t5_block)   */
...
as (                                                                                           
    ...
    ORDER SIBLINGS BY src.flow_name, a.TOP_LEVEL_EXECUTION_AUDIT_ID desc,  (updated_on - created_on) DESC --a.p80_DURATION_MINS DESC 
