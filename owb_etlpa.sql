...
--
-----------------------------------------------------------
drop table MONITOR_DW.owb_etlpa_tmp1;

create table monitor_dw.owb_etlpa_tmp1
as
with fl_execs
as(
  select  t2.flow_name,
...
select * from monitor_dw.owb_etlpa_tmp1;

-----------------------------------------------------------
--
-- *** STEP 2: Find Critical Path for the specific flows
--
-----------------------------------------------------------
...
drop table monitor_dw.owb_etlpa_tmp2;

create table monitor_dw.owb_etlpa_tmp2
as
 select *
  from (
          ...
          from (
              ...
              from (
                  with q1 as ( 
                      ...
                      where
                          TOP_LEVEL_EXECUTION_AUDIT_ID in (select top_level_execution_audit_id from monitor_dw.owb_etlpa_tmp1)
                  )                                                                                           
...
exec dbms_stats.gather_table_stats('MONITOR_DW', upper('owb_etlpa_tmp2'))

select * from monitor_dw.owb_etlpa_tmp2;
...
drop table monitor_dw.owb_etlpa_tmp3;

create table monitor_dw.owb_etlpa_tmp3
as
...
with q1
  as(
    ...
    from monitor_dw.owb_etlpa_tmp2
  ),
...
from q3
order by flow_name, duration_mins desc;

/*
select regexp_substr('  CHECK_FLOWSEND_FORGLOBAL_PROC [370, p80-370, TYPE:Procedure] ', '\S+\s\[(\d+\.?\d?)',1,1,'i',1) from dual;
*/

exec dbms_stats.gather_table_stats('MONITOR_DW', upper('owb_etlpa_tmp3'))

select * from monitor_dw.owb_etlpa_tmp3;

-----------------------------------------------------------
--
-- *** STEP 3: Detailed Analysis per node ( for long running nodes)
--
--  For each Node in the CP that exceeds a certain time threshold
...
--    from (
        select * 
        from monitor_dw.owb_etlpa_tmp3
        where type in ('Procedure', 'Mapping', 'ProcessFlow', 'Function', 'Shell')
            and duration_mins > 15 -- 15 minutes threshold
...
as (                                                                                           
    ...
    ORDER SIBLINGS BY a.TOP_LEVEL_EXECUTION_AUDIT_ID desc, (updated_on - created_on) DESC 
